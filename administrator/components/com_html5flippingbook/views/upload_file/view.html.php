...
class HTML5FlippingBookViewUpload_File extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->dir 					= $jinput->get('dir', '', 'STRING');
		$this->pubid                = $jinput->getInt('pubid');
		$this->extensionsStr 		= $jinput->get('extensions', '', 'STRING');
		$this->elementId 			= $jinput->get('elementId', '');
		$this->linkedElementIdsStr 	= $jinput->get('linkedElementIds', '', 'STRING');
	...
	private function upload($dir, $extensionsStr, $elementId, $linkedElementIds, $publicationId)
	{
		...
		if ($isImage && $elementId == 'jform_page_image')
		{
			...
			$targetDirFullOriginalIMG = $baseDir . '/original-' . $publicationId;
